...
% threshold = threshold of meticulous the edgedetection should be.
% Recomended 0.7
% centers = the centercoords of noteheads. If the function finds several
% all are returned as an array
% radius = the corresponding radius to the centers
%
binImg = getBinImg(img, 1);
imshow(binImg)
[centers,radius, metric] = imfindcircles(binImg, r, 'ObjectPolarity','bright', 'Method','TwoStage', 'EdgeThreshold', threshold);
[centers, i] = sortrows(centers, 1);
%%TODO fix radius order